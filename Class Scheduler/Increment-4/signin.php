...
else
{
	if($_SERVER['REQUEST_METHOD'] != 'POST')
	{
		echo '
		 ...
		 <div id="footer">
			...
			<br> <a style="text-align:center;" href="forgot.php" class="button2">Forgot Password?</a>
		 </div>
		 </center>';
	}
	else
	{
		...
		{
			$sql = "SELECT 	user_id,user_name,user_level,professorid  FROM	$userstable WHERE user_name = '" . mysql_real_escape_string($_POST['user_name']) . "' 	AND user_pass = '" . sha1($_POST['user_pass']) . "'";
					...
					while($row = mysql_fetch_assoc($result))
					{
						if($row['user_level']=="2") 
						{
							...
							{								
								...
								<br><br><br><a href="index.php" class="button2">Proceed to the Main Page</a><br><br>';
							}else if( $row['user_level']==9)
							
							{								
								echo "<script>window.setTimeout('"; echo 'window.location="admin/index.php"; '; echo "',1000);</script>";
								echo '<center>Welcome Manager, ' . $_SESSION['user_name'] . '. <br /><br>You will be redirected in a second. <br>If not click link below.
								<br><br><br><a href="admin/index.php" class="button2">Proceed to the Main Page</a><br><br>';
							}
